...
def PsiPartitionPlus(msa: str, format: str, alphabet: str, asc: str, k: int, w: dict = None):
    ...
    with open(Path(msa).with_suffix('.log'), 'w') as log_file:
        ...
        if asc == "no":
            default_iqtree([
                ...
                '-spp', Path(msa).with_suffix('.parts'), '-m', 'MFP', '-mrate', 'E,I,G,I+G,R,I+R', '-nt', '4',],
                log_file)
        elif asc == "yes":
            default_iqtree([
                '-s', msa, '-pre', Path(msa).with_suffix(''),
            ...
            ValueError('Invalid ASC specification')
...
def opt_func(args) -> float:
    wandb.init(project='PsiPartitionPlus')
    msa = args.msa
    format = args.format
    ...
    stat = PsiPartitionPlus(msa, format, alphabet, asc, k, w)
...
if __name__ == '__main__':
    ...
    if not tuning_csv.exists():
        ...
        sweep_id = wandb.sweep(sweep_configuration, project='PsiPartitionPlus')
